...
export function PaymentTrendChart({ data, title = "Payment Trends", description = "Payment collections over time" }: Props) {
      ...
      <CardContent>
        <ChartContainer config={chartConfig}>
          <BarChart accessibilityLayer data={data} margin={{ top: 20 }}>
            ...
            <ChartTooltip
              cursor={false}
              content={<ChartTooltipContent hideLabel formatter={(value) => [`₱${Number(value).toLocaleString()}`, "Collections"]} />}
            />
            <Bar dataKey="amount" fill="var(--color-collections)" radius={8} name="Collections">
